...
def weights_const(i, o, name, const, keepdims=False):
    W_values = np.ones(_get_shape(i, o, keepdims)).astype(
        theano.config.floatX) * const
...
def weights_identity(i, o, name, const, keepdims=False):
    ...
    W_values = np.eye(*_get_shape(i, o, keepdims)
                      ).astype(theano.config.floatX) * const
    return theano.shared(value=W_values, name=name, borrow=True)
...
def weights_Glorot(i, o, name, rng, is_logistic_sigmoid=False, keepdims=False):
    ...
    W_values = rng.uniform(
        low=-d, high=d, size=_get_shape(i, o, keepdims)).astype(theano.config.floatX)
    return theano.shared(value=W_values, name=name, borrow=True)
...
def load(file_path, minibatch_size, x, p=None):
    ...
    gsums = [theano.shared(gsum)
             for gsum in state["gsums"]] if state["gsums"] else None
...
class GRULayer(object):
    ...
    def __init__(self, rng, n_in, n_out, minibatch_size):
        ...
        self.h0 = theano.shared(value=np.zeros((minibatch_size, n_out)).astype(
            theano.config.floatX), name='h0', borrow=True)
    ...
    def step(self, x_t, h_tm1):
        ...
        rz = T.nnet.sigmoid(T.dot(x_t, self.W_x) +
                            T.dot(h_tm1, self.W_h) + self.b)
        r = _slice(rz, self.n_out, 0)
        z = _slice(rz, self.n_out, 1)

        h = T.tanh(T.dot(x_t, self.W_x_h) +
                   T.dot(h_tm1 * r, self.W_h_h) + self.b_h)
...
class GRU(object):
    ...
    def __init__(self, rng, x, minibatch_size, n_hidden, x_vocabulary, y_vocabulary, stage1_model_file_name=None, p=None):
        ...
        if os.path.exists(pretrained_embs_path):
            ...
            # UNK - both quite arbitrary initializations
            We.append([0.0] * n_emb)
            ...
            self.We = weights_Glorot(x_vocabulary_size, n_emb, 'We', rng)

        self.GRU_f = GRULayer(rng=rng, n_in=n_emb,
                              n_out=n_hidden, minibatch_size=minibatch_size)
        self.GRU_b = GRULayer(rng=rng, n_in=n_emb,
                              n_out=n_hidden, minibatch_size=minibatch_size)

        # output model
        self.GRU = GRULayer(rng=rng, n_in=n_hidden * 2,
                            n_out=n_hidden, minibatch_size=minibatch_size)
        ...
        def output_recurrence(x_t, h_tm1, Wa_h, Wa_y, Wf_h, Wf_c, Wf_f, bf, Wy, by, context, projected_context):
            ...
            fw = T.nnet.sigmoid(T.dot(lfc, Wf_f) +
                                T.dot(h_t, Wf_h) + bf)  # fusion weights
        ...
        x_emb = self.We[x.flatten()].reshape(
            (x.shape[0], minibatch_size, n_emb))

        [h_f_t, h_b_t], _ = theano.scan(
            fn=input_recurrence,  # forward and backward sequences
            sequences=[x_emb, x_emb[::-1]],
            outputs_info=[self.GRU_f.h0, self.GRU_b.h0])

        # 0-axis is time steps, 1-axis is batch size and
        # 2-axis is hidden layer size
        context = T.concatenate([h_f_t, h_b_t[::-1]], axis=2)
        projected_context = T.dot(context, self.Wa_c) + self.ba

        [_, self.last_hidden_states, self.y, self.alphas], _ = \
            theano.scan(
                fn=output_recurrence,
                # ignore the 1st word in context,
                # because there's no punctuation before that
                sequences=[context[1:]],
                non_sequences=[self.Wa_h, self.Wa_y, self.Wf_h, self.Wf_c,
                               self.Wf_f, self.bf, self.Wy, self.by, context,
                               projected_context],
                outputs_info=[self.GRU.h0, None, None, None])

        print "Number of parameters is %d" % \
            sum(np.prod(p.shape.eval()) for p in self.params)

        self.L1 = sum(abs(p).sum() for p in self.params)
        self.L2_sqr = sum((p**2).sum() for p in self.params)

    def cost(self, y):
        # time steps * number of parallel sequences in batch
        num_outputs = self.y.shape[0] * self.y.shape[1]
...
class GRUstage2(GRU):
    ...
    def __init__(self, rng, x, minibatch_size, n_hidden, x_vocabulary, y_vocabulary, stage1_model_file_name, p=None):
        ...
        # output model
        self.GRU = GRULayer(rng=rng, n_in=self.stage1.n_hidden + 1,
                            n_out=n_hidden, minibatch_size=minibatch_size)
        self.Wy = weights_const(n_hidden, y_vocabulary_size, 'Wy', 0)
        self.by = weights_const(1, y_vocabulary_size, 'by', 0)
        ...
        def recurrence(x_t, p_t, h_tm1, Wy, by):
            ...
            h_t = self.GRU.step(x_t=T.concatenate(
                (x_t, p_t.dimshuffle((0, 'x'))), axis=1), h_tm1=h_tm1)
        ...
        [_, self.y], _ = theano.scan(fn=recurrence,
                                     sequences=[
                                         self.stage1.last_hidden_states, p],
